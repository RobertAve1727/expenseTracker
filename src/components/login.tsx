...
const Login = ({ onToggle }: { onToggle: () => void }) => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        // useAuth internally handles sessionStorage and the 'auth-change' event
        // navigate() automatically handles the HashRouter prefix
        navigate("/dashboard");
      }
    } catch (err) {
  ...
  return (
    <main className="flex-1 flex flex-col justify-center px-4 lg:px-20 py-12">
      <div className="w-full max-sm mx-auto lg:mx-0">
