  ...
  return (
    <main className="flex-1 flex flex-col justify-center px-4 lg:px-20 py-12">
      <div className="w-full max-w-sm mx-auto lg:mx-0">
        ...
        {error && (
          <div className="mt-4 text-xs font-bold text-red-500 bg-red-500/10 p-4 rounded-sm border border-red-500/20">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSignIn} className="mt-8 space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full h-[52px] bg-flow-button text-white rounded-sm font-semibold text-sm hover:opacity-90 active:scale-[0.98] transition-all shadow-lg flex justify-center items-center gap-2"
          >
            {isLoading ? (
              "Verifying..."
            ) : (
